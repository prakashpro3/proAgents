...
`;
    ...
    // Always include universal AGENTS.md (codex platform)
    const platformsWithUniversal = selectedPlatforms.includes('codex')
      ? selectedPlatforms
      : [...selectedPlatforms, 'codex'];

    // Copy AI instruction files for selected platforms (merges with existing files)
    const aiResults = copyPlatformFiles(platformsWithUniversal, sourceDir, targetDir);
...
async function smartUpdate(sourceDir, targetDir) {
  ...
  const aiResults = copyPlatformFiles(platformsWithUniversal, sourceDir, projectRoot);
